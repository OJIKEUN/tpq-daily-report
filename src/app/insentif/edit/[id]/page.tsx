...
export default function EditInsentifPage() {
  ...
  const [teachers, setTeachers] = useState<FormTeacher[]>([]);
  ...
  const fetchLetter = async () => {
    try {
      const d = await getDoc(doc(db, 'incentive_letters', id));
      if (d.exists()) {
        const data = d.data();
        setNomor(data.nomor || '');
        ...
        setStartMonth(data.periode_start_month ?? 3);
        setEndMonth(data.periode_end_month ?? 5);
        setPeriodeYear(data.periode_year ?? now.getFullYear());
        setTanggalPengesahan(data.tanggal_pengesahan || now.toISOString().split('T')[0]);
        setPenandaTangan(data.penanda_tangan || 'Sugiarti');
        if (data.teachers && data.teachers.length > 0) {
          setTeachers(data.teachers);
        } else {
          setTeachers([{ type: 'registered', user_id: '', name: '', no_sk: '', no_ktp: '', no_rek: '' }]);
        }
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateDoc(doc(db, 'incentive_letters', id), {
        nomor,
        lampiran,
        perihal,
        kepada,
      ...
      console.error(error);
      alert('Gagal memperbarui surat insentif.');
    } finally {
    ...
    <div className="flex-1 flex flex-col min-h-screen bg-base-200">
      <div className="navbar bg-base-100 shadow-sm sticky top-0 z-40 border-b border-base-200">
        ...
        <div className="flex-1">
          <h1 className="text-lg font-bold">Edit Surat Insentif</h1>
        </div>
      </div>
